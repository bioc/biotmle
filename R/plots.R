...
heatmap_ic <- function(x, ..., design, FDRcutoff = 0.25,
  ...
  # the EIF contributions: biomarkers along rows, subjects along columns. The
  # rows are keyed by the row names of the tmleOut table, which is what the
  # topTable "ID" column is matched against below.
  eif_mat <- if (any(class(x@tmleOut) %in% "EList")) {
    as.matrix(x@tmleOut$E)
  } else {
    as.matrix(x@tmleOut)
  }
  rownames(eif_mat) <- rownames(x@tmleOut)
